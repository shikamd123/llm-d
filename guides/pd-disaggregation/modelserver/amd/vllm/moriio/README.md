...
A small ZMQ "registration proxy" Deployment (`moriio-zmq-proxy.yaml`)
is included so the overlay is deployable without extra out-of-tree
manifests. It is pinned (`hostNetwork`) to a services node and reached
by that node's IP (`REPLACE_ZMQ_PROXY_IP`), which is robust for the
`hostNetwork` worker pods regardless of cluster DNS.
...
| `REPLACE_SERVICES_NODE`           | Hostname of the node the zmq registration proxy is pinned to.                  |
| `REPLACE_ZMQ_PROXY_IP`            | That node's IP (port 10001), set as `proxy_ip` in the prefill/decode patches.  |
